...
interface GraphNode {
  id: string
  type: 'research' | 'tool' | 'technical'
  title: string
  description: string
  url: string
  image?: string
  coords: GraphNodeCoords
}
...
const TYPE_ORDER: NodeType[] = ['research', 'tool', 'technical']
...
const state: Record<NodeType, TypeState> = {
  research: { active: true, nodes: [], colors: [] },
  tool: { active: true, nodes: [], colors: [] },
  technical: { active: true, nodes: [], colors: [] },
}

const plotEl = document.getElementById('plot') as HTMLDivElement
...
function buildControls(): {
  ...
  const pills = {} as Record<NodeType, HTMLButtonElement>
...
async function applySearch(query: string): Promise<number> {
  const q = query.trim().toLowerCase()
  let totalMatches = 0
  const restylePromises: Array<Promise<unknown>> = []
...
function toggleType(type: NodeType, button: HTMLButtonElement): void {
...
function badgeStyle(type: NodeType): string {
...
function showTooltip(node: GraphNode): void {
...
async function init(): Promise<void> {
  ...
  ;(plotEl as any).on('plotly_hover', (event: any) => {
    if (pinnedNodeId) {
      return
    }
    ...
    const node = point.customdata as GraphNode
    showTooltip(node)
  })

  ;(plotEl as any).on('plotly_unhover', () => {
    if (pinnedNodeId) {
      return
    }
    hideTooltip()
  ...
  ;(plotEl as any).on('plotly_click', (event: any) => {
    const point = event?.points?.[0]
    if (!point?.customdata) {
      return
    }
